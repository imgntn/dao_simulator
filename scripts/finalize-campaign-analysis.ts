...
import { finalizeCampaignAnalysis } from '../lib/research/analysis-finalization';
...
export function analysisFinalizationUsage(): string {
  return [
    'Usage:',
    '  npm run campaign:finalize-analysis -- --campaign <verified-dir>',
    '    --copy <new-read-only-dir> --experiment-ids <id[,id...]> [options]',
    '',
    'Options:',
    '  --output <dir>       Independent source and copied-campaign analyses',
    '  --report <file>      Final machine-readable identity comparison',
    '  --help               Print this message',
  ].join('\n');
}

export function parseAnalysisFinalizationArgs(argv: string[]): Arguments {
...
function main(): void {
  ...
  const args = parseAnalysisFinalizationArgs(process.argv.slice(2));
  if (args.help) {
    console.log(analysisFinalizationUsage());
    return;
  }
  ...
  const outputDir = path.resolve(
    rootDir,
    args.output ?? path.join('results', 'analysis-finalization', campaignId),
  );
  const reportPath = path.resolve(
    rootDir,
    args.report ?? path.join(outputDir, 'finalization-report.json'),
  );
  const report = finalizeCampaignAnalysis({
    rootDir,
    campaignDir,
  ...
  console.log(
    `Analysis finalization passed for ${report.campaignId}: `
    + `sha256:${report.sourceAnalysisIdentitySha256}`
    + (report.supportingBundle
      ? `, supporting bundle sha256:${report.supportingBundle.sourceIdentitySha256}`
      : ''),
  );
}
