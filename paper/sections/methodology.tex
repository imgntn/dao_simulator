...
\begin{tabular}{llll}
\toprule
Experiment & Parameter & Values & Runs/Config \\
\midrule
% AUTO-UPDATED: experiment list
Quorum Sensitivity & \texttt{quorumConfig.baseQuorumPercent} & 1-20 & 50 \\
DAO Scale & \texttt{memberDistribution.totalMembers} & 25-500 & 50 \\
Voting Systems & \texttt{votingSystem.type} & token, quad, conviction & 100 \\
Participation & \texttt{simulationParams.votingActivity} & 0.1-0.8 & 50 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item Hardware: {{HARDWARE_SPEC}}
    \item Concurrency: 4-8 parallel simulations
    \item Total compute time: {{TOTAL_COMPUTE_TIME}}
    \item Total simulation runs: 0
\end{itemize}
