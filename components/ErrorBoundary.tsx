...
export class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  render(): ReactNode {
    if (this.state.hasError) {
      ...
      return (
        <div className="flex flex-col items-center justify-center min-h-[200px] p-6 bg-[var(--surface-warm)] rounded-lg border border-[var(--border-default)]">
          <div className="text-red-500 mb-2">
            <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
          </div>
          <h3 className="text-lg font-semibold text-[var(--text-heading)] mb-1">Something went wrong</h3>
          <p className="text-[var(--text-body)] text-sm text-center mb-4 max-w-md">
            {this.state.error?.message || 'An unexpected error occurred'}
          </p>
          <button
            onClick={this.handleReset}
            className="px-4 py-2 bg-[var(--accent-teal)] hover:bg-[var(--accent-teal-hover)] text-white rounded-lg transition-colors"
          >
            Try again
          </button>
        </div>
...
export class Canvas3DErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  render(): ReactNode {
    if (this.state.hasError) {
      return (
        <div className="flex flex-col items-center justify-center h-full min-h-[400px] p-6 bg-[var(--surface-warm-deep)] rounded-lg border border-[var(--border-default)]">
