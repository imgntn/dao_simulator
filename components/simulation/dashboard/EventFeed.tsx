...
const EVENT_COLORS: Record<string, string> = {
  ...
  proposal_expired: 'text-gray-500',
  vote_cast: 'text-gray-400',
...
export function EventFeed({ className = '' }: Props) {
  ...
  return (
    <div className={`bg-gray-950/80 backdrop-blur-sm border-t border-gray-800 p-3 ${className}`}>
      <div className="space-y-0.5 max-h-[160px] overflow-y-auto">
        {visible.map((evt, i) => (
          <div key={`${evt.step}-${i}`} className="flex items-start gap-2 text-xs leading-relaxed">
            <span className="font-mono text-gray-600 min-w-[3ch] text-right">{evt.step}</span>
            <span className={`${EVENT_COLORS[evt.type] ?? 'text-gray-400'} min-w-[1ch]`}>
