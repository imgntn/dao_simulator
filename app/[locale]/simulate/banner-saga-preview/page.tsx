...
/**
 * Preview page for BannerSagaScene — renders the scene with mock data
 * so you can see the design without wiring into the live simulation.
 *
 * Open at: /en/simulate/banner-saga-preview
 */

import { useMemo, useState } from 'react';
import { BannerSagaScene } from '@/components/simulation/banner-saga';
import type {
  AgentSnapshot,
  ProposalSnapshot,
  SimulationSnapshot,
...
export default function BannerSagaPreviewPage() {
    ...
    <div className="flex h-screen w-screen flex-col" style={{ background: '#c9b896' }}>
      ...
      <div className="flex items-center justify-between gap-3 border-b border-black/20 bg-stone-100 px-3 py-2 text-xs text-stone-700">
        <div className="flex items-center gap-3">
          <strong>Banner Saga preview</strong>
      ...
      <div className="min-h-0 flex-1">
        <BannerSagaScene snapshot={snapshot} />
      </div>
    </div>
  );
}
