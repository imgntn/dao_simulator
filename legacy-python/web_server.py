...
class WebServer:
    ...
    def _register_routes(self) -> None:
        ...
        async def snapshot() -> Dict[str, Any]:
            """Return a minimal authoritative snapshot of the world for visualization clients."""
            self._ensure_simulation()
            dao = self.sim.dao
            # Agents
            agents = [
                {
                    'id': getattr(a, 'unique_id', f'agent_{i}'),
                    'type': a.__class__.__name__,
                    'tokens': getattr(a, 'tokens', 0),
                    'reputation': getattr(a, 'reputation', 0),
                    'guild': getattr(getattr(a, 'guild', None), 'name', None),
                }
                for i, a in enumerate(dao.members)
            ]
            # Proposals
            proposals = [
                {
                    'id': getattr(p, 'unique_id', f'proposal_{i}'),
                    'title': getattr(p, 'title', f'Proposal {i}'),
                    'status': getattr(p, 'status', 'open'),
                    'funding_goal': getattr(p, 'funding_goal', 0),
                    'current_funding': getattr(p, 'current_funding', 0),
                    'creation_time': getattr(p, 'creation_time', 0),
                    'voting_period': getattr(p, 'voting_period', getattr(p, 'duration', 0)),
                }
                for i, p in enumerate(dao.proposals)
            ]
            # Projects
            projects = [
                {
                    'title': getattr(pr, 'title', f'Project {i}'),
                    'progress': float(getattr(pr, 'total_work', lambda: 0)() or 0),
                    'duration': getattr(pr, 'duration', 0),
                    'start_time': getattr(pr, 'start_time', 0),
                    'funding_goal': getattr(pr, 'funding_goal', 0),
                    'current_funding': getattr(pr, 'current_funding', 0),
                }
                for i, pr in enumerate(dao.projects)
            ]
            return {
                'type': 'snapshot',
    ...
    async def _send_initial_state(self, ws: WebSocket) -> None:
        ...
        for i, proposal in enumerate(self.sim.dao.proposals):
            await ws.send_text(json.dumps({
                ...
                'step': self.sim.schedule.steps,
                'proposal_id': getattr(proposal, 'unique_id', f'proposal_{i}'),
    ...
    def _on_event(self, **data: Any) -> None:
        ...
        # Enhance and normalize event data for visualization
        enhanced_data = dict(data)
        event_name = enhanced_data.get('event')
        # Ensure "step" is present
        if 'step' not in enhanced_data and self.sim:
            enhanced_data['step'] = self.sim.schedule.steps

        # Helper: map title -> proposal and inject proposal_id + funding fields
        def attach_proposal_fields_from_title(title: str) -> None:
            if not self.sim or not title:
                return
            try:
                p = next((p for p in self.sim.dao.proposals if getattr(p, 'title', None) == title), None)
                if p is None:
                    return
                if 'proposal_id' not in enhanced_data:
                    enhanced_data['proposal_id'] = getattr(p, 'unique_id', None)
                enhanced_data.setdefault('funding_goal', getattr(p, 'funding_goal', None))
                enhanced_data.setdefault('current_funding', getattr(p, 'current_funding', None))
            except Exception:
                pass
        ...
        # Map member-like fields to a common agent identifier when present
        for agent_key in ('member', 'investor', 'provider', 'validator', 'violator'):
            if agent_key in enhanced_data and 'agent_id' not in enhanced_data:
                enhanced_data['agent_id'] = enhanced_data.get(agent_key)
        
        # Normalize and add proposal metadata
        if 'proposal_id' in enhanced_data and self.sim:
            ...
            try:
                ...
                if isinstance(proposal_id, int) and 0 <= proposal_id < len(self.sim.dao.proposals):
                    ...
                    # Look up by string unique_id
                    proposal = next(
                        (p for p in self.sim.dao.proposals if getattr(p, 'unique_id', None) == proposal_id),
                        None,
                    )
                if proposal is not None:
                    enhanced_data.setdefault('funding_goal', getattr(proposal, 'funding_goal', 1000))
                    enhanced_data.setdefault('current_funding', getattr(proposal, 'current_funding', 0))
            except (ValueError, IndexError):
                pass
        else:
            # Fallback: if only a title-like field is present, attach proposal fields
            title = enhanced_data.get('title') or enhanced_data.get('proposal')
            if title:
                attach_proposal_fields_from_title(title)

        # Special handling for proposal_created (ensure proposal_id)
        if event_name == 'proposal_created' and 'proposal_id' not in enhanced_data:
