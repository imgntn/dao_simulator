...
class TestStaking(unittest.TestCase):
    ...
    def test_member_specific_interest_compound(self):
        sim = DAOSimulation(
            num_developers=1,
            num_investors=0,
            num_delegators=0,
            ...
            num_passive_members=0,
            comment_probability=0,
            staking_interest_rate=0.05,
        )
        member = sim.dao.members[0]
        member.staking_rate = 0.2
        member.compound_stake = True
        member.stake_tokens(50, "DAO_TOKEN")
        sim.step()
        self.assertAlmostEqual(member.staked_tokens, 60)
    ...
    def test_partial_slash_severity(self):
        from data_structures import DAO, Dispute
        from agents import DAOMember, Arbitrator
        ...
        dao.add_member(member)
        dao.add_member(arbitrator)
        dispute = Dispute(dao, [member], "issue", importance=0.5, project=None, member=member)
        dao.add_dispute(dispute)
        member.stake_tokens(40, "DAO_TOKEN")
        arbitrator.arbitrate(dispute)
        expected = 40 - 40 * dao.slash_fraction * dispute.importance
        self.assertAlmostEqual(member.staked_tokens, expected)
